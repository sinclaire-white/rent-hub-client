...
const AddRentPostsPage = () => {
  ...
  if (session?.user) {
    setForm((prev) => ({
      ...
      ownerName: `${session.user.firstName || ""} ${session.user.lastName || ""}`.trim(),
    }));
  }
}, [session]);
          ...
          <div className="flex flex-col gap-6">
            ...
            <div>
              ...
              <input
                ...
                readOnly
                className="w-full border border-base-200 rounded-lg px-3 py-2 text-base-content bg-base-100 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-not-allowed"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-base-content mb-1">
            ...
            <div>
              <label className="block text-sm font-medium text-base-content mb-1">
                Rent Price (৳/
                {["Vehicles", "Tools & Equipment", "Events & Venues"].includes(
                  form.category
                )
                  ? "day"
                  : "month"}
...
function MapSelector({ latitude, longitude, onSelect }) {
  ...
  useEffect(() => {
    ...
    function loadMap() {
      ...
      map = window.L.map(mapRef.current, { closePopupOnClick: false }).setView(
        [latitude, longitude],
        16
      );
